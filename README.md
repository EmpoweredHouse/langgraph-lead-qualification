...
```mermaid
graph TD;
    A[Company Research Data (JSON) + ICP + Buyer Persona] -->|Verify Data| B[Lead Qualification Agent];
    B -->|Check Missing Info| C[Web Research];
    C -->|Retrieve Data| B;
    B -->|Complete Data| D[Lead Scoring & Reasoning];
    D -->|Output| E[Qualified Lead JSON];
```
